...
run:if-cmd python3 source:file "$XDG_CONFIG_HOME/dotfiles/configs/pythonrc.sh"
if iscmd fzf; then
    export FZF_DIR="$HOMEBREW_PREFIX/opt/fzf"
    path-append "$FZF_DIR/bin"
    source:file "$XDG_CONFIG_HOME/dotfiles/configs/fzf-rc.sh"
    # @audit removed plugin -- still want to use it?
    # source:file "$XDG_CONFIG_HOME/dotfiles/configs/forgit-rc.sh"
fi
